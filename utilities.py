...
def distance_matrix(clients):
    matrix = list()
    for c in clients:
        row = list()
        for cl in clients:
            distance = ((cl.longitude - c.longitude) ** 2 + (cl.latitude - c.latitude) ** 2) ** 0.5
            row.append(distance)
        matrix.append(row)
    return matrix

def distance_to_centers(districts, clients):
    matrix = list()
    for district in districts:
        row = list()
        for cl in clients:
            distance = ((cl.longitude - district.center.longitude) ** 2 + (cl.latitude - district.center.latitude) ** 2) ** 0.5
            row.append(distance)
        matrix.append(row)
    return matrix
...
#    instance.name = splitted_line[1]
    ...
    for i in range(nodes):
        f1 = f.readline()
        splitted_line = f1.split()
        client = Client(int(splitted_line[0]),
...
class MapVisualiser:
    ...
    def draw_cluster(self, solution):
        ...
        focus_lat = solution.districts[0].center.latitude
        focus_long = solution.districts[0].center.longitude
        ...
        for district in solution.districts:
            ...
            latitude_list = []
            longitude_list = []
            for client in district.clients:
                latitude_list.append(client.latitude)
